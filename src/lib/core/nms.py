...
def pose_nms(cfg, heatmap_avg, poses):
    ...
    # Convert all tensors to the same device as heatmap_avg
    heatmap_avg = heatmap_avg.cpu()  # Move heatmap_avg to CPU if it's on GPU
    poses = [pose.cpu() for pose in poses]  # Move all poses to CPU
